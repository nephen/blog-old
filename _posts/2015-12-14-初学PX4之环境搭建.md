...
下面是具体该怎么[操作](https://pixhawk.org/dev/simulation/native_sitl)，最新的文档请到[这里](http://dev.px4.io/simulation-sitl.html)，建议参考[这篇](https://pixhawk.org/dev/hil/jmavsim)。
...
# => Qt配置如下：
# => 配置QT5动态共享库：
~ $ exportline="export QTDIR=/opt/Qt5.5.1/5.5/gcc_64"
~ $ echo $exportline >> ~/.profile
~ $ exportline="export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${QTDIR}/lib"
~ $ echo $exportline >> ~/.profile
~ $ source ~/.profile
# => 更换qt源，默认为qt4
~ $ export QT_SELECT=qt5
# => 更改文件/usr/lib/x86_64-linux-gnu/qtchooser/5.conf 第一行为：/opt/Qt5.5.1/5.5/gcc_64/bin
